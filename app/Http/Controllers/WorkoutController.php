    ...
    public function index(Request $request)
{
    ...
    $workoutsQuery = Workout::with([
        'bodyparts',
        'equipments',
        'ratings' => function ($query) {
            $query->select('workout_id', 'user_id', 'rating');
        }
    ])
    ...
    ->map(function ($workout) {
        $userRated = $workout->ratings->contains('user_id', Auth::id());
        $averageRating = $workout->ratings->avg('rating');
        $ratingsCount = $workout->ratings->groupBy('user_id')->count(); // Count distinct users who have rated
